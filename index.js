...
fs.writeFileSync('./database/user.json', JSON.stringify(db_menfes))} 
...
if (pesan_temanh.length <1) return reply(`Harus di isi semua !!\nex : ${command} 62857xxx|nama|hallo`)
if (!q) return reply(`Format Fitur Confes / Kirim pesan rahasia ke seseorang Lewat bot\n\n_Example_\n${prefix+command} wa|pengirim|pesan\n\n_Contoh_\n${command} 6285789004732|crush|hello\n\n*Note :*\nBerawal dari 628xxx Tanpa Spasi`)
lexxy.sendMessage(`${nomor_temanh}@s.whatsapp.net`, { text: `━━━[ *PESAN-RAHASIA* ]━━━\n_Hi ada confess nih buat kamu_\n\n*dari :* ${ini_nama_kamu}\n*pesan :* ${pesan_temanh}\n\n_Pesan ini di tulis oleh seseorang,_\n_bot hanya menyampaikan saja._\n━━━━━━━━━━━━━━━━`, footer: 'klik button untuk membalas pesan', buttons: [{buttonId: `${prefix}balas_confes ${nomor_pengirimnyah}@s.whatsapp.net|${nomor_temanh}@s.whatsapp.net`, buttonText: {displayText: 'balas✍️'}, type: 1}, {buttonId: `${prefix}kirkonn ${sender.split("@")[0]}|${nomor_pengirimnyah}|${ini_nama_kamu}`, buttonText: {displayText: '➳sendkontak'}, type: 1}],headerType: 1 }, {quoted: msg})
reply('Sukses mengirimkan pesan ke dia.')
}
break
case prefix+'balas_confes':{
...
let button_menfes = [{ buttonId: `${prefix}balas_menfes ${nomor_pengirimnya}@s.whatsapp.net|${nomor_teman}@s.whatsapp.net`, buttonText: { displayText: "Balas✍️" }, type: 1 }, {buttonId: `${prefix}kirkonn ${sender.split("@")[0]}|${nomor_pengirimnya}|${nama_pengirim}`, buttonText: {displayText: '➳sendkontak'}, type: 1}]
const ini_mess_menfess = { image: await reSize(setting.pathimg, 300, 200), caption: text_menfess, footer: 'klik button untuk membalas pesan', buttons: button_menfes, headerType: 4 }
const sendMsg = await lexxy.sendMessage(`${nomor_teman}@s.whatsapp.net`, ini_mess_menfess, { quoted })
